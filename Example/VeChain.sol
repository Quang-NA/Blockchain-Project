*/ Vechain Smartcontract:

- Tạo 2 smart contract gồm TokenSale và Token. Chuyển quyền sở hữu của Token sang TokenSale.
- TokenSale:
	- Chia 2 loại: non-publicToken (4 nhóm) và publicToken(2 nhóm).
	- Có biến thể hiện các giai đoạn của ICO.
	- Giá token cố định theo ETH, có thưởng thêm %.

- Token:
	- Total Supply ban đầu là 0.
	- Mỗi account có 2 kiểu token: balance, raw(được tạo ra khi user bỏ ETH ra mua token)
	- Có hàm mint (hàm tạo token), có biến bool để xác định kiểu token.
	- Token không bán hết sẽ được thưởng thêm cho user theo % raw nắm giữ.
	- Balance của user = balance + raw.
*/
