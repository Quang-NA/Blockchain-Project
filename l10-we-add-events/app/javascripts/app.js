...
window.App = {
    ...
    listenToEvents: function() {
        MyWallet.deployed().then(function(instance) {
            instance.receivedFunds({},{fromBlock:0, toBlock:'latest'}).watch(function(error, event) {
                document.getElementById("fundEvents").innerHTML += JSON.stringify(event.args);
                // console.log(event);
            });
            instance.proposalReceived({},{fromBlock:0, toBlock:'latest'}).watch(function(error, event) {
                document.getElementById("proposalEvents").innerHTML += JSON.stringify(event.args);
    ...
    confirmTransaction: function() {
        var _proposalID = parseInt(document.getElementById("proposalID").value);

        MyWallet.deployed().then(function(instance) {
            return instance.confirmProposal(_proposalID, {from: accounts[0], gas: 500000});
    ...
    submitTransaction: function() {
        ...
        // MyWallet.deployed().then(function(instance) {
        //     // Hàm deployed() trả lại instance của MyWallet. then() sử dụng instance này để thực hiện tác vụ
        //     // (vd: hàm spendMoneyOn() có giá trị trả về là txHash). Nếu muốn sử dụng giá trị trả về để
        //     // thực hiện các tác vụ khác, thì sử dụng RETURN.
        //
        //     // return instance.spendMoneyOn(_to, web3.toWei(_amount, 'finney'), _reason, {from:accounts[0]});
        //     instance.spendMoneyOn(_to, web3.toWei(_amount, 'finney'), _reason, {from:accounts[0]});
        // });
